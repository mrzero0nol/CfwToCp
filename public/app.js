    ...
    async function init(){
        ...
        window.addEventListener('popstate', function(event) {
            ...
            if (actives.length > 0) {
                actives.forEach(el => { if(el.id !== 'authModal') el.classList.remove('active'); });
            }
        });
    }
    ...
    // --- AUTH SYSTEM (LOGIN/REGISTER) ---
        ...
        function navigateTo(pageId, el) {
            ...
            if(pageId === 'historyPage') filterHist('all');
            if(pageId === 'assetsPage') renderAssetsPage();
        ...
        function updateCartCount() {
            ...
            if(count > 0) { bad1.style.display='flex'; bad1.innerText=count; bad2.style.display='flex'; bad2.innerText=count; }
        ...
        function openCart() { navigateTo('cartPage', document.querySelectorAll('.nav-item')[3]); closeModal('productDetailPage'); }
        ...
        function removeFromCart(idx) { cart.splice(idx, 1); saveCart(); renderCartPage(); }
        ...
        async function processBuy(){
            ...
            const r = await fetch('/api/buy', {method:'POST', headers:{'Content-Type': 'application/json'}, body:JSON.stringify({code:currentCode, qty:q, voucherCode:null})});
        ...
        async function checkStatus(){
            ...
            const d=await r.json();
            if(d.status==='PAID'){
                const c=await fetch('/api/claim',{method:'POST', headers:{'Content-Type': 'application/json'}, body:JSON.stringify({oid:currentOrderId})});
        ...
        function switchAdminTab(t){
            ...
            if(t==='vouc') { renderVoucherTargetOptions(); loadAdminVouchers(); }
        ...
        async function admSaveConfigSilent(){
             ...
             await fetch('/api/admin/save-config',{method:'POST',headers:{'Content-Type': 'application/json', 'Admin-Key':localStorage.getItem('adminKey')},body:JSON.stringify(b)});
        }
        ...
        async function admSaveConfig(){
            ...
            const res = await fetch('/api/admin/save-config',{method:'POST',headers:{'Content-Type': 'application/json', 'Admin-Key':localStorage.getItem('adminKey')},body:JSON.stringify(b)});
        ...
        function goToHome(){
             navigateTo('homePage', document.querySelectorAll('.nav-item')[0]);
        ...
        async function doLogin(){ const p=document.getElementById('adminPass').value; const r=await fetch('/api/admin/login',{method:'POST',headers:{'Content-Type': 'application/json'},body:JSON.stringify({password:p})}); const d=await r.json(); if(d.success){ localStorage.setItem('adminKey',p); closeModal('loginModal'); checkAdminSession(); toast("Login OK"); } else toast("Salah"); }
        ...
        async function admAddStockDirect(){ const v=document.getElementById('newStockDirect').value; await fetch('/api/admin/stock-action',{method:'POST',headers:{'Content-Type': 'application/json', 'Admin-Key':localStorage.getItem('adminKey')},body:JSON.stringify({code:activeStockCode,action:'add',data:v})}); document.getElementById('newStockDirect').value=''; admFetchStock(activeStockCode, document.getElementById('stkModalName').innerText); toast("Stok Ditambah"); loadProducts(); }
        async function admDelStockDirect(i){ if(!confirm("Hapus?"))return; await fetch('/api/admin/stock-action',{method:'POST',headers:{'Content-Type': 'application/json', 'Admin-Key':localStorage.getItem('adminKey')},body:JSON.stringify({code:activeStockCode,action:'delete',index:i})}); admFetchStock(activeStockCode, document.getElementById('stkModalName').innerText); loadProducts(); }
        async function admAdd(){ await fetch('/api/admin/add-product',{method:'POST',headers:{'Content-Type': 'application/json', 'Admin-Key':localStorage.getItem('adminKey')},body:JSON.stringify({code:document.getElementById('nCode').value,name:document.getElementById('nName').value,price:document.getElementById('nPrice').value,category:document.getElementById('nCat').value,img:document.getElementById('nImg').value,desc:document.getElementById('nDesc').value,stockData:document.getElementById('nStock').value})}); toast("Disimpan"); loadProducts(); }
        ...
        function renderVoucherTargetOptions() {
            const s = document.getElementById('vTarget');
            s.innerHTML = '<option value="ALL">Semua Produk</option>';
            allProducts.forEach(p => s.innerHTML += `<option value="${p.code}">${p.name}</option>`);
        }
        async function loadAdminVouchers(){const r=await fetch('/api/admin/vouchers',{headers:{'Admin-Key':localStorage.getItem('adminKey')}});const d=await r.json();const l=document.getElementById('voucherListAdmin');l.innerHTML='';if(d.vouchers.length===0){l.innerHTML='<p style="text-align:center;color:#888;">Belum ada voucher</p>';return}
        d.vouchers.forEach(v=>{
            const tName = v.validFor === 'ALL' ? 'Semua Produk' : (allProducts.find(x=>x.code===v.validFor)?.name || v.validFor);
            const usage = `${v.used||0} / ${(!v.limit || v.limit===0) ? '∞' : v.limit}`;
            l.innerHTML+=`<div class="stock-item" style="display:flex;justify-content:space-between; flex-direction:column; align-items:flex-start; gap:5px;">
                ...
                <div style="font-size:0.75rem; color:var(--text-muted); width:100%; display:flex; justify-content:space-between;">
                    <span>Target: ${tName}</span>
                    <span>Terpakai: ${usage}</span>
                </div>
            </div>`
        })}
        async function admSaveVoucher(){const c=document.getElementById('vCode').value.trim().toUpperCase(),a=parseInt(document.getElementById('vAmount').value),t=document.getElementById('vTarget').value,l=parseInt(document.getElementById('vLimit').value)||0;if(!c||!a)return toast("Lengkapi Data");await fetch('/api/admin/save-voucher',{method:'POST',headers:{'Content-Type': 'application/json', 'Admin-Key':localStorage.getItem('adminKey')},body:JSON.stringify({code:c,amount:a,validFor:t,limit:l})});toast("Voucher Dibuat");loadAdminVouchers()}
        async function admDelVoucher(c){if(!confirm("Hapus?"))return;await fetch('/api/admin/del-voucher',{method:'POST',headers:{'Content-Type': 'application/json', 'Admin-Key':localStorage.getItem('adminKey')},body:JSON.stringify({code:c})});toast("Dihapus");loadAdminVouchers()}
        async function admDelete(){const c=document.getElementById('editProdSelect').value;if(!c)return;if(!confirm("Hapus produk ini?"))return;await fetch('/api/admin/delete-product',{method:'POST',headers:{'Content-Type': 'application/json', 'Admin-Key':localStorage.getItem('adminKey')},body:JSON.stringify({code:c})});toast("Terhapus");location.reload()}
        async function admSave(){const c=document.getElementById('editProdSelect').value;if(!c)return;await fetch('/api/admin/edit-product',{method:'POST',headers:{'Content-Type': 'application/json', 'Admin-Key':localStorage.getItem('adminKey')},body:JSON.stringify({code:c,price:document.getElementById('editPrice').value,category:document.getElementById('editCat').value,img:document.getElementById('editImg').value,desc:document.getElementById('editDesc').value})});toast("Update Sukses");loadProducts()}
